    ...
    if (data) {
      if (data.Levels == true) {
        ...
        if (hasLeveledUp) {
          ...
          const levelUp = await new soycanvas.LevelUp()
          ...
          if (messageData) {
            ...
            try {
              if (levelData) {
                await client.channels.cache.get(levelData.Channel)
                  .send({ content: levelMessage, files: [{ attachment: levelUp, name: `levelup-${message.member.id}.png` }] })
                  .catch(() => { });
              } else {
                await message.channel.send({ content: levelMessage, files: [{ attachment: levelUp, name: `levelup-${message.member.id}.png` }] });
              }
            } catch {
              await message.channel.send({ content: levelMessage, files: [{ attachment: levelUp, name: `levelup-${message.member.id}.png` }] });
            }
            ...
            try {
              if (levelData) {
                await client.channels.cache.get(levelData.Channel).send({ content: `**GG** <@!${message.author.id}>, you are now level **${user.level}**`, fles: [{ attachment: levelUp, name: `levelup-${message.member.id}.png` }]}).catch(() => { });
              } else {
                message.channel.send({ content: `**GG** <@!${message.author.id}>, you are now level **${user.level}**`, fles: [{ attachment: levelUp, name: `levelup-${message.member.id}.png` }] });
              }
            } catch {
              message.channel.send({ content: `**GG** <@!${message.author.id}>, you are now level **${user.level}**`, fles: [{ attachment: levelUp, name: `levelup-${message.member.id}.png` }] });
            }
